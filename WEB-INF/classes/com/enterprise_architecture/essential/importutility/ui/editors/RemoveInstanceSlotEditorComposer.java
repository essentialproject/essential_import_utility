 ...
 *
 */
public class RemoveInstanceSlotEditorComposer extends EssentialImportInterface {

	private ImportSpecDataManager importSpecDataManager;
	
	public Div removeInstanceSlotEditorDiv;
	public Listbox instanceListBox;
	...
	private RemoveInstanceSlotScript currentInstanceSlot;
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		IdSpace window = removeInstanceSlotEditorDiv.getSpaceOwner();
        ...
        if(!currentData.getClass().getName().endsWith("RemoveInstanceSlotScript")) {
        	importspectree.setModel(importspectree.getModel());
        	return;
        }
		this.currentInstanceSlot = (RemoveInstanceSlotScript) currentData;
		...
		//initialise the list of instance variables
		variableNameList = new ArrayList();
        
		//add the list of global instance variables
		List<DerivedInstanceType> globInstances = importSpecDataManager.getSpreadsheetImportSpecScriptData().getGlobalInstances().getDerivedInstance();
		Iterator<DerivedInstanceType> globInstIter = globInstances.iterator();
		while(globInstIter.hasNext()) {
			DerivedInstanceType globInst = globInstIter.next();
			variableNameList.add(globInst.getVariableName());
		}
		...
		if(parentData.getClass().getName().equals("com.enterprise_architecture.essential.importutility.data.common.script.SimpleInstanceTypeScript")) {
			SimpleInstanceTypeScript parentSimpleInstance = (SimpleInstanceTypeScript) parentData;
			variableNameList.addAll(importSpecDataManager.getSimpleInstanceOlderSiblingVars(parentSimpleInstance, wsImportSpec));
		} else {
			DerivedInstanceTypeScript parentDerivedInstance = (DerivedInstanceTypeScript) parentData;
			variableNameList.addAll(importSpecDataManager.getDerivedInstanceOlderSiblingVars(parentDerivedInstance, wsImportSpec));
		}
		
		if(variableNameList.size() == 0) {
			variableNameList.add(0, "- No Instances Available -");
		} else {
			variableNameList.add(0, "- Select Instance Variable -");
		}
		ListModelList model = new ListModelList(variableNameList);
		
        
		instanceListBox.setModel(model);
        
		instanceListBox.addEventListener("onSelect", new EventListener(){
            public void onEvent(Event event) throws Exception {
            	//refresh the import spec tree
            	int selectionIndex = instanceListBox.getSelectedIndex();
            	if(selectionIndex > 0) {
	            	ListModel listModel = instanceListBox.getModel();
	            	String selectedVariable = (String) listModel.getElementAt(selectionIndex);
	            	currentInstanceSlot.setInstanceSlotVar(selectedVariable);
	            	oldSelectedVariableIndex = selectionIndex;
            		...
            		Messagebox.show("An instance variable must be selected", "Set Instance Variable Error", Messagebox.OK, Messagebox.ERROR);
            		instanceListBox.setSelectedIndex(oldSelectedVariableIndex);
            	}
            }
        });
		...
		if(this.currentInstanceSlot != null) {
			...
			//pre-select the variable of the current Instance Slot in the list
			String currentVarName = this.currentInstanceSlot.getInstanceSlotVar();
	        String aVar;
	        Iterator<String> varIter = variableNameList.iterator();
			if(currentVarName != null) {
				while(varIter.hasNext()) {
					aVar = (String) varIter.next();
					if((aVar != null) && (aVar.equals(currentVarName))) {
						int instIndex = variableNameList.indexOf(aVar);
						instanceListBox.setSelectedIndex(instIndex);	
						oldSelectedVariableIndex = instIndex;
						break;
					}
				}
			}
