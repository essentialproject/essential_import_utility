/**
	...
	* 
	* Controller class to manage the implementation of each service request
...
/**
 * Class to implement the service. Using the controller pattern, validates the request parameters
...
public class ImportServiceImpl 
{
	...
	/**
	 * Return code indicating that an invalid worksheet was requested
	 */
	public static final int INVALID_WORKSHEET = 440;
	...
	/**
	 * Default constructor which initialises the thread pool.
	 */
								...
								boolean isPublishAfterImport)
	{
		...
		ImportJob anImportJob = new ImportJob();
		...
		anImportJob.setBatchID(batchID);
		anImportJob.setResponseURL(responseURL);
		anImportJob.setServletContext(context);
		anImportJob.setUserID(IUUsername);
		anImportJob.setPassword(IUPassword);
		anImportJob.setPublishAfterImport(isPublishAfterImport);
		...
		if(aReturnCode == JOB_ACCEPTED)
		{
			...
			try
			{
				Future aTaskHandle = itsExecutor.submit(anImportJob); 
			}
