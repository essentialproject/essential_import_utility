/**
	...
	* 18.08.2016	JWC Version 1.1 exclude operation of service in EIP mode
	* 13.03.2017	JWC	Version 1.2 improved logging
 */
 ...
 * existing Import Activities to be invoked automatically.
...
{	
	/**
	 * Application pararmeter maintaining the singletone for the Import Service controller
	 * 
	 */
	public static final String CONTROLLER_SINGLETON_PARAM = "controller";
	...
	public void destroy()
	{
		...
		ImportServiceImpl aController = getImportController(false);
		if(aController != null)
		{
		...
		else // Service available as we're not in EIP mode
		{
			...
			if(aContentType.contains(MULTIPART_FORM_CONTENT_TYPE))
			{
				...
				synchronized(theRequest)
				{
					...
					{
						//System.err.println("Import Service: Exception encountered while parsing request to receive uploaded source content.");
						//System.err.println(aFileUploadEx.getMessage());
						iuLog.log(Level.WARNING, "Import Service: Exception encountered while parsing request to receive uploaded source content.", aFileUploadEx);
						isSuccess = false;
					...
					{
						//System.err.println("Import Service: Exception encountered while writing received CSV / Excel file.");
						//System.err.println(aWritingEx.getMessage());
						iuLog.log(Level.WARNING, "Import Service: Exception encountered while writing received CSV / Excel file.", aWritingEx);
						isSuccess = false;
						aResponseCode = HttpServletResponse.SC_INTERNAL_SERVER_ERROR; 
				...
				//System.out.println("ImportService: Received File: Content Type = " + aFileContentType);
				String aLogMessage = "ImportService: Received File: Content Type = " + aFileContentType;
				iuLog.log(Level.INFO, aLogMessage);
			...
			if(isSuccess)
			{
				...
				ImportServiceImpl aController = getImportController();
			...
			else
			{
				switch(aReturnCode)
				{
					...
					case ImportServiceImpl.INVALID_USER :
						...
						break;
					case ImportServiceImpl.INVALID_WORKSHEET:
						aResponseCode = HttpServletResponse.SC_NOT_FOUND;
	 ...
	 */
	protected ImportServiceImpl getImportController()
	{
		...
		ImportServiceImpl aController = (ImportServiceImpl)itsServletContext.getAttribute(aSingleton);
		
		// If we get a null back, then the Singleton has not been created. Create it
		if(aController == null)
		{
			aController = new ImportServiceImpl();
	 ...
	 */
	protected ImportServiceImpl getImportController(boolean theIsCreate)
	{
		if(theIsCreate)
		{
	...
	protected synchronized String saveSourceContent(ZipInputStream theZippedInStream, String theFilename, String theSheetName)
	{
		...
		catch(IOException anXMLFileEx)
		{
			...
			iuLog.log(Level.WARNING, "Essential Report Service: IO Exception while reading received Zip archive. Details:", anXMLFileEx);
			isSuccess = false;
		}
		catch(NullPointerException anNPE)
		{
			...
			iuLog.log(Level.WARNING, "Essential Report Service: Exception reading received Zip archive. Details:", anNPE);
			isSuccess = false;
		}
		catch(Exception anZipFileEx)
		{
			...
			iuLog.log(Level.WARNING, "Essential Report Service: Exception reading received Zip archive. Details:", anZipFileEx);
			isSuccess = false;
		}
	...
	protected synchronized String saveSourceContent(FileItem theUncompressedItem, String theFilename)
	{
		...
		{
			//System.err.println("Exception writing received Excel source file");
			//System.err.println(ioEx.getMessage());
			//ioEx.printStackTrace(new PrintWriter(System.err));
			iuLog.log(Level.WARNING, "Exception writing received Excel source file", ioEx);
			isSuccess = false;
		}		
	...
	protected synchronized String saveSourceCSVContent(FileItem theUncompressedCSV, String theFilename, String theSheetName)
	{
		...
		{
			//System.err.println("Exception writing converted CSV source file");
			//System.err.println(ioEx.getMessage());
			//ioEx.printStackTrace(new PrintWriter(System.err));
			iuLog.log(Level.WARNING, "Exception writing converted CSV source file", ioEx);
			isSuccess = false;
		}		
	...
	/**
	 * Canned test harness for the Callback functionality
	 * @return
	 */
	private ImportCallback buildTestResponse()
	{
		...
		try
		{
			aJobDetails.setBatchId("12345");
			aJobDetails.setImportActivity("Example Import");
			aJobDetails.setSourceContent("testfile.csv.xlsx");
			aJobDetails.setTargetEnvironment("Example Target Env");
			...
			aJobStatus.setValue("success");
			aJobStatus.setMessage("import completed");
			aJobDetails.setStatus(aJobStatus);
			anImportResults.setImportJob(aJobDetails);
			
			StatusType aPubStat = new StatusType();
			aPubStat.setValue("success");
			aPubStat.setMessage("published to viewer sucessfully");
			aPublishDetails.setStatus(aPubStat);
			aPublishDetails.setRequested("yes");
