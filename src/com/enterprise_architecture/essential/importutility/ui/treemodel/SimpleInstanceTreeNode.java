 ...
 * 31.01.2019	JWC Fix bug with one constructor after upgrade to ZK 8.5
 * 
 */
 ...
 *
 */
public class SimpleInstanceTreeNode extends DefaultTreeNode {
	// Node Control the default open
    private boolean open = false;
 
    public SimpleInstanceTreeNode(Object data, List children, boolean open) {
        super(data, children);
    ...
    public SimpleInstanceTreeNode(Object data, List children) {
        super(data, children);
        if(children != null && data != null) {
        	initChildren();
        }
    ...
    public SimpleInstanceTreeNode(Object data) {
        // 31.01.2019 JWC Resolving issue with instances
    	// Ensure that the child list is not null
    ...
    public boolean isLeaf() {
    	SimpleInstanceTypeScript instanceScript = (SimpleInstanceTypeScript) getData();
    	return (instanceScript.getInstanceSlotOrRemoveInstanceSlotOrDeleteInstanceSlot().size() == 0);
    }
    
    private void initChildren() {
    	SimpleInstanceTypeScript simpleInstance = (SimpleInstanceTypeScript) getData();
    	Iterator slotsIter = simpleInstance.getInstanceSlotOrRemoveInstanceSlotOrDeleteInstanceSlot().iterator();
