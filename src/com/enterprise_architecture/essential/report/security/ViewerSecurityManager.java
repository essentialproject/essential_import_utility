 ...
 * 06.05.2021	JWC Tweaks for library upgrade of OauthUtils 1.7.1
 */
 ...
 *********************************************************************************************/
	...
	public String authenticateUserBySession(HttpServletRequest theRequest, HttpServletResponse theResponse)
	{
		...
		String aUserXml = getUserXmlFromSession(theRequest, theResponse);
	...
	private String getUserXmlFromSession(HttpServletRequest theRequest, HttpServletResponse theResponse)
	{
		...
		if (OauthTokenConsumerUtils.hasBearerTokenOrRefreshToken(theRequest)) {
			try {
				...
				aUserXml = (String)theRequest.getSession().getAttribute(USER_SESSION_VAR);
				...
				if(aUserXml == null)
				{
					 ...
					 */
					OauthBearerTokenClaims aClaims = OauthTokenConsumerUtils.getVerifiedClaimsFromBearerToken(aFreshBearerToken);
			 ...
			 * This method will return null which will redirect the user to the login page.
			 */
			itsLog.info("No bearer token or refresh token, can be caused by user manually logging out which removes the OAuth token cookies.");
		}
